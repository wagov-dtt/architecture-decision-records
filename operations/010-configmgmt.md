...
1. **Git Repository Structure**: Single repo per application with `environments/{dev,int,uat,prod}` folders matching AWS account names (e.g., `app-a-infra` repo → `app-a-dev`, `app-a-int`, `app-a-uat`, `app-a-prod` accounts)
...
4. **Versioning**: Git tags = semantic versions (x.y.z) created on `main` and deployable to UAT and PROD
...
Git tags represent immutable release versions for UAT and PROD. DEV and
INT may deploy approved branch refs per [ADR 009](../development/009-release.md).
Environment folders (`environments/{dev,int,uat,prod}`) map to separate AWS
accounts with isolated state storage.
